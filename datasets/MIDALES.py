...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Class handling MIDALES dataset.
...
class MIDALESDataset(PointCloudDataset):
    """Class to handle MIDALES dataset."""

    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'MIDALES')
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for MIDALES data: ', self.set)
        ...
        train_names, test_names = self.prepare_MIDALES_ply()
        ...
        if self.set == 'training':
            for i in range(len(train_names)):
                self.files += [join(ply_path, 'train', train_names[i] + ".ply")]

            self.cloud_names = train_names

        elif self.set in ['validation', 'test', 'ERF']:
            for i in range(len(test_names)):
                self.files += [join(ply_path, 'test', test_names[i] + ".ply")]

            self.cloud_names = test_names
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        while True:
            ...
            input_features = input_intensity.astype(np.float32)
    ...
    def prepare_MIDALES_ply(self, from_txt=False):
        """
        ...
        #get list of train and test files from the directory.txt file
        with open(self.path + "/directory.txt") as f:
            train_files = []
            test_files = []
            l = f.readline()
            while ("#test" not in l):
        ...
        train_path = join(self.path, "train")

        output_path = join(ply_path, "train")

        # check if the directory for training ply files exists, if not yell at user
        if not exists(output_path):
            print("no training files")

        # check if the directory for test ply files exists, if not yell at user
        if not exists(output_path):
            print("no test files")
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                sub_intensity = data['intensity'].astype(np.float32)
...
class MIDALESSampler(Sampler):
    """Sampler for MIDALES"""

    def __init__(self, dataset: MIDALESDataset):
...
class MIDALESCustomBatch:
    """Custom batch definition with memory pinning for MIDALES"""
...
def MIDALESCollate(batch_data):
    return MIDALESCustomBatch(batch_data)
