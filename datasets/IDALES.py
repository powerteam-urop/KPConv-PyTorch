...
class IDALESDataset(PointCloudDataset):
    """Class to handle IDALES dataset."""

    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'IDALES')
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for IDALES data: ', self.set)
        ...
        train_names, test_names = self.prepare_IDALES_ply()
    ...
    def prepare_IDALES_ply(self, from_txt=False):
        """
...
class IDALESSampler(Sampler):
    """Sampler for IDALES"""

    def __init__(self, dataset: IDALESDataset):
...
class IDALESCustomBatch:
    """Custom batch definition with memory pinning for IDALES"""
...
def IDALESCollate(batch_data):
    return IDALESCustomBatch(batch_data)
