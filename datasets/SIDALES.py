...
class SIDALESDataset(PointCloudDataset):
    """Class to handle SIDALES dataset."""

    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'SIDALES')
        ...
        train_names, test_names = self.prepare_SIDALES_ply()
    ...
    def prepare_SIDALES_ply(self, from_txt=False):
        """
...
class SIDALESSampler(Sampler):
    """Sampler for SIDALES"""

    def __init__(self, dataset: SIDALESDataset):
...
class SIDALESCustomBatch:
    """Custom batch definition with memory pinning for SIDALES"""
...
def SIDALESCollate(batch_data):
    return SIDALESCustomBatch(batch_data)
