...
class ADALESDataset(PointCloudDataset):
    """Class to handle ADALES dataset."""

    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'ADALES')
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for ADALES data: ', self.set)
        ...
        train_names, test_names = self.prepare_ADALES_ply()
    ...
    def prepare_ADALES_ply(self, from_txt=False):
        """
        ...
        # check if the directory for training ply files exists, if not make one
        ...
        # check if the directory for test ply files exists, if not make one
        if not exists(output_path):
            mkdir(output_path)
...
class ADALESSampler(Sampler):
    """Sampler for ADALES"""

    def __init__(self, dataset: ADALESDataset):
...
class ADALESCustomBatch:
    """Custom batch definition with memory pinning for ADALES"""
...
def ADALESCollate(batch_data):
    return ADALESCustomBatch(batch_data)
